...
- **reference/troubleshooting.md**: Troubleshooting for LLM setup, hooks, and shell integration in Codex or general terminal workflows
...
1. **Detect available tools**
   ```bash
   which codex llm aichat claude 2>/dev/null
   ```

2. **If none are installed, recommend Codex or another supported CLI**
   - Prefer Codex in this package because the surrounding guidance is written for Codex users
   - Fall back to another supported tool from `reference/llm-commits.md` only if that better matches the user's environment
...
- User's project instructions or explicit instruction authorizes this pattern
