...
[commit.generation]
command = "codex exec --json -s read-only - | jq -sr '[.[] | select(.item.type? == \"agent_message\")] | last.item.text'"
```
...
### Codex (recommended for this plugin)
...
Requires `jq`. This command uses the user's configured default Codex model instead of pinning a version in the Worktrunk config.
...
- **Agent tracking** — This Codex port can set markers automatically when the documented hook subset is available; otherwise use manual marker commands
