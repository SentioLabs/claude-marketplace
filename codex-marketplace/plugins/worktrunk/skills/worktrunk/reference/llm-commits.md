...
Uses the current Codex CLI without pinning a model name, so it respects the user's configured default model. Requires `jq` for JSON parsing. A local validation probe for this port confirmed that `codex exec --json ...` emits an `agent_message` event that can be extracted this way.
