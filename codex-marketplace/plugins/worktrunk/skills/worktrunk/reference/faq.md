...
If shell integration isn't working (auto-cd not happening, completions missing, `wt` not found as a function), the fastest path to a fix is to inspect the config and shell files directly:

1. Run `wt config show`
2. Inspect the relevant shell config files (`~/.bashrc`, `~/.zshrc`, fish functions, or PowerShell profiles)
3. Compare the setup against `reference/shell-integration.md`
4. If you're using this Codex plugin, ask Codex to walk through the output and shell config with you

If the issue still isn't clear, please [open an issue](https://github.com/max-sixty/worktrunk/issues/new?title=Shell%20setup%20issue&body=%23%23%20Shell%20and%20OS%0A%0A-%20Shell%3A%20%0A-%20OS%3A%20%0A%0A%23%23%20Output%20of%20%60wt%20config%20show%60%0A%0A%60%60%60%0A%0A%60%60%60%0A%0A%23%23%20What%20you%20checked%0A%0A) with the output of `wt config show`, the shell (bash/zsh/fish), and OS. Non-standard success and failure cases are both useful for making Worktrunk easier to set up for others.
