...
This command stores in mneme's memory system — searchable, scoreable, and visible in the web UI. This complements agent-specific local memory features (for example Claude Code auto-memory). Use runtime-local memory for session context and mneme for the shared knowledge base.

For Codex-oriented transparent memory workflows, use a conservative capture policy:

- store explicit preferences
- store explicit decisions
- store durable project facts and conventions
- avoid storing ephemeral task chatter or temporary debugging notes
