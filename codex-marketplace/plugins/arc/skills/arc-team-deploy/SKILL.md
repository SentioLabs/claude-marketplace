...
Deploy an agent team from arc's issue graph. This workflow is currently grounded in Claude Code team primitives and translates `teammate:*` labels, plans, and dependencies into a coordinated team with tasks and role-filtered context.
...
Codex installs this skill for documentation parity, but the workflow below is not yet Codex-native.

The current deployment flow still depends on Claude Code primitives that are not yet confirmed in Codex: `TeamCreate`, `TaskCreate`, `TaskUpdate`, `TaskList`, `Agent`, and `SendMessage`.

In Codex, treat this skill as a reference for the intended workflow and run the actual team deployment from Claude Code until equivalent Codex primitives are available.
